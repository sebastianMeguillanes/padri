	...
	tabla=$('#tbllistado').DataTable(
        {
            ...
            "ajax":
                    {
                        url: '../ajax/user_personal.php?op=0',
                        type : "get",
                        dataType : "json",						
                        error: function(e){
...
function guardaryeditar()
{
	...
	$.ajax({
		url: "../ajax/user_personal.php?op=1",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
{
	$.post("../ajax/user_personal.php?op=4",{idusuario : idusuario}, function(data, status)
	{
		console.log(data);
		data = JSON.parse(data);		
		mostrarform(true);
        
		$("#nombre").val(data.personalnombre);
		$("#email").val(data.personalemail);
		$("#num_documento").val(data.identificacionpersonal);
		$("#telefono").val(data.personalcelular);
		$("#direccion").val(data.personaldireccion);
		$('#rol').val(data.idrol);
		$('#departamento').val(data.iddepartamento);
		$("#idusuario").val(data.id_usuariop);
		$("#login").val(data.nombreusuariop);
        ...
        $.post("../ajax/user_personal.php?op=6",{clave : data.contraseñausuario}, function(r){
			$("#clave").val(r);
		});
 	});
}
...
function desactivar(idusuario)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/user_personal.php?op=2", {idusuario : idusuario}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
...
function activar(idusuario)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/user_personal.php?op=3", {idusuario : idusuario}, function(e){
                console.log(e);
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
